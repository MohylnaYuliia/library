...
public class LibraryServiceTestImpl {
    ...
    @Transactional
    @Rollback
    public void testWhenUserReturnsAllBooks() {
        BookEntity book = BookEntity.builder().id(FIRST_BOOK_ID).name(FIRST_BOOK_NAME).existed(false).copy(0).build();
        BookEntity bookSecond = BookEntity.builder().id(SECOND_BOOK_ID).name(SECOND_BOOK_NAME).existed(true).copy(1).build();
        bookRepository.save(book);
        bookRepository.save(bookSecond);
    ...
    @Transactional
    @Rollback
    public void testWhenUserReturnBooksWhenUserDoNotBorrowedBooks() {
        bookRepository.save(BookEntity.builder().id(FIRST_BOOK_ID).name(FIRST_BOOK_NAME).existed(true).copy(1).build());
        userRepository.save(UserEntity.builder().id(FIRST_USER_ID).name("John").build());
    ...
    @Transactional
    @Rollback
    public void testWhenUserReturnsBooksThatNotBorrowed() {
        BookEntity book = BookEntity.builder().id(FIRST_BOOK_ID).name(FIRST_BOOK_NAME).existed(true).copy(1).build();
        BookEntity bookSecond = BookEntity.builder().id(SECOND_BOOK_ID).name(SECOND_BOOK_NAME).existed(true).copy(1).build();
        bookRepository.save(book);
        bookRepository.save(bookSecond);
